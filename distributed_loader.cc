...
future<size_t>
distributed_loader::load_new_sstables(sstables::sstable_directory& dir, sharded<database>& db,
    ...
    return do_with(dht::ring_position::max(), dht::ring_position::min(), [&table, &dir, &view_update_generator, datadir] (dht::ring_position& min, dht::ring_position& max) {
        return dir.do_for_each_sstable([&table, datadir, &min, &max] (sstables::shared_sstable sst) {
            ...
            return sst->move_to_new_dir(datadir.native(), gen,  true).then([&table, sst] {
                table._sstables_opened_but_not_loaded.push_back(sst);
                return make_ready_future<>();
            });
        }).then([&table, &view_update_generator, &min, &max] {
            // nothing loaded
            ...
            std::exchange(table._sstables_opened_but_not_loaded, {});
            return make_exception_future<size_t>(std::move(ep));
        });
    });
}
...
distributed_loader::process_upload_dir(distributed<database>& db, distributed<db::system_distributed_keyspace>& sys_dist_ks,
    ...
    return seastar::async(std::move(attr), [&db, &view_update_generator, &sys_dist_ks, ks = std::move(ks), cf = std::move(cf)] {
        ...
        reshape(directory, db, 4, global_table->schema()->max_compaction_threshold(), ks, cf, [global_table, upload, &shard_gen] (shard_id shard) {
            auto gen = shard_gen[shard].fetch_add(smp::count, std::memory_order_relaxed);
            return global_table->make_sstable(upload.native(), gen,
                  global_table->get_sstables_manager().get_highest_supported_format(),
        ...
        size_t loaded = directory.map_reduce0([&db, ks, cf, datadir, &view_update_generator] (sstables::sstable_directory& dir) {
            return load_new_sstables(dir, db, view_update_generator, datadir, ks, cf);
...
future<> distributed_loader::populate_column_family(distributed<database>& db, sstring sstdir, sstring ks, sstring cf) {
    return async([&db, sstdir = std::move(sstdir), ks = std::move(ks), cf = std::move(cf)] {
        ...
        unsigned max_sstables = offstrategy_threshold * 2;
        reshape(directory, db, offstrategy_threshold, max_sstables, ks, cf, [global_table, sstdir, sst_version] (shard_id shard) {

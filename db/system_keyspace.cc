...
static future<> build_bootstrap_info() {
    ...
    return execute_cql(req, LOCAL, sstring(LOCAL)).then([] (auto msg) {
        ...
        if (!msg->empty() && msg->one().has("bootstrapped")) {
            state = std::unordered_map<sstring, bootstrap_state>({
            ...
            }).at(msg->one().template get_as<sstring>("bootstrapped"));
        }
...
future<std::unordered_set<dht::token>> update_local_tokens(
    ...
    return get_saved_tokens().then([&add_tokens, &rm_tokens] (auto tokens) {
...
future<> set_bootstrap_state(bootstrap_state state) {
    sstring state_name = std::unordered_map<bootstrap_state, sstring, enum_hash<bootstrap_state>>({

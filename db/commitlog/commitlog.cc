...
public:
    ...
    segment_manager(config c)
        ...
        , _max_memory(cfg.max_active_writes * (128 <<  10))
...
//        logger.info("Starting a cicle one way or another {}\n", *this);
        if (_buffer.empty()) {
            printf("Cycling an empty buffer\n");
        ...
        return _pending_ops.run_with_ordered_post_op(rp, [this, size, off, buf = std::move(buf)]() mutable {
            auto p = buf.get();
            size_t written = 0;
            auto gate = make_lw_shared<seastar::gate>();
...
//                        printf("\tPartially completed cycle of size %ld, memory used %ld, pending writes %ld\n", bytes, _segment_manager->region_group.memory_used(), 
//                                _segment_manager->totals.pending_writes);
            ...
            return gate->close().finally([this, buf = std::move(buf), size, gate]() mutable {
                _segment_manager->release_buffer(std::move(buf));
                ++_segment_manager->totals.cycle_count;
                --_segment_manager->totals.pending_writes;
//                _segment_manager->region_group.update(-size);
//                logger.info("Fully completed cycle for {}, memory used {}\n"
//                        , *this, _segment_manager->region_group.memory_used());
            });
    ...
    replay_position do_allocate(const cf_id_type& id, shared_ptr<entry_writer> writer, size_t s, size_t size) {
        _gate.enter();
//        logger.info("Started do allocate for {}, current buffer {}, size {}, memory_usage {}", *this, _buffer.size(), s, _segment_manager->region_group.memory_used());

        if (!_buffer.empty() && (s > (_buffer.size() - _buf_pos))) { // enough data?
            ...
            cycle(_segment_manager->cfg.mode == sync_mode::BATCH);
        }
...
scollectd::registrations db::commitlog::segment_manager::create_counters() {
    ...
    return {
        ...
        add_polled_metric(type_instance_id("commitlog"
                        , per_cpu_plugin_instance, "delay", "pending_allocations_max")
                        , make_typed(data_type::GAUGE, [this] {
                            return region_group.blocked_for();
                        })
        ),
        add_polled_metric(type_instance_id("commitlog"
                        , per_cpu_plugin_instance, "queue_length", "pending_allocations")
                        , make_typed(data_type::GAUGE, [this] {
                            return region_group.blocked_requests();
                        })
        ),
    };
}

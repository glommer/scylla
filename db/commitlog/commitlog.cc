...
future<> db::commitlog::segment_manager::shutdown() {
    if (!_shutdown) {
        ...
        auto seg_gate = _gate.close().then(std::bind(&segment_manager::sync_all_segments, this, true));
        // Now that the gate is closed we are sure nobody else will pop()

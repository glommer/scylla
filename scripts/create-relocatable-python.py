...
def fix_python_binary(ar, binpath):
    ...
    patched_binary = mkstemp()[1]
    shutil.copy2(binpath, patched_binary)
...
def dependencies(package_list):
    ...
    output = subprocess.check_output(['repoquery',
                '--archlist=noarch,x86_64', # We don't want i686 packages, since we will only install lib64/
                '--cacheonly',              # Don't look into the yum cache. Guarantees consistent builds
                '--installed',
                '--resolve',
                '--requires',
...
def generate_file_list(executables):
    ...
    return [ x for x in candidates if file_to_copy(x) ]

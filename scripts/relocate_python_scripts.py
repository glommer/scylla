...
b="$(basename "$x")"
d="$(dirname "$x")"
extrapath="$d/$(realpath --relative-to ${{d}} {pythonpath} 2>/dev/null)"
PYTHONPATH="${{d}}:${{d}}/libexec:$PYTHONPATH" PATH="${{extrapath}}:${{PATH}}" exec -a "$0" "${{d}}/libexec/${{b}}" "$@"
'''
        self.thunk = thunk.format(pythonpath=os.path.dirname(python_path))

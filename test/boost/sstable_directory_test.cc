...
SEASTAR_TEST_CASE(sstable_directory_test_table_lock_works) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        sstdir.invoke_on_all([&e, ks_name, cf_name] (sstable_directory& dir) {
            auto& table = e.local_db().find_column_family(ks_name, cf_name);
            dir.store_phaser(table.write_in_progress());
            return make_ready_future<>();
        }).get();
...
SEASTAR_TEST_CASE(sstable_directory_shared_sstables_reshard_correctly) {
    ...
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto generation = 0;
        auto msb = e.db().local().get_config().murmur3_partitioner_ignore_msb_bits();
        for (unsigned nr = 0; nr < num_sstables; ++nr) {
            ...
            auto sst = cf.make_sstable(upload_path.native(), generation++, sstables::sstable::version_types::mc, sstables::sstable::format_types::big);

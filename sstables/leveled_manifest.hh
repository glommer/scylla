...
public:
    ...
    static constexpr unsigned sstables_per_run = 10;
...
public:
    ...
    static uint64_t max_bytes_for_level(int level, uint64_t max_sstable_size_in_bytes) {
        ...
        double bytes = pow(sstables_per_run, level) * max_sstable_size_in_bytes;
    ...
    sstables::compaction_descriptor get_compaction_candidates(const std::vector<stdx::optional<dht::decorated_key>>& last_compacted_keys,
        ...
        // L0 is okay, and yet we seem to have nothing to run. Try pushing things to the highest
        // level
        for (size_t i = 1; i < _generations.size() - 1; i++) {
            auto& sstables = get_level(i);
            if (sstables.empty()) {
                continue;
            }
            auto& sstables_next_level = get_level(i+1);
            // I am the de facto next level
            if (sstables_next_level.empty()) {
                continue;
            }
            auto descriptor = get_descriptor_for_level(i, last_compacted_keys, compaction_counter);
            if (descriptor.sstables.size() > 0) {

...
class size_tiered_backlog_tracker final: public compaction_backlog_tracker::impl {
    ...
    std::unordered_set<seastar::lw_shared_ptr<sstable_write_progress>> _in_progress = {};
    std::unordered_set<seastar::lw_shared_ptr<compaction_read_progress>> _in_compaction = {};
    ...
    inflight_component partial_backlog() {
        ...
        for (auto& swp:  _in_progress) {
            if (swp->bytes_written) {
                in.total_bytes += swp->bytes_written;
                in.contribution += swp->bytes_written * log4(swp->bytes_written);
            }
        }
        return in;
    }
    ...
    inflight_component compacted_backlog() {
        ...
        for (auto& crp:  _in_compaction) {
            if (crp->sstable_size > 0) {
                in.total_bytes += crp->bytes_compacted;
                in.contribution += crp->bytes_compacted * log4(crp->sstable_size);
            }
        }
        return in;
    }
...
public:
    ...
    virtual void register_partially_written_sstable(lw_shared_ptr<sstable_write_progress> wp) override {
        _in_progress.insert(std::move(wp));
    }

    virtual void seal_partially_written_sstable(lw_shared_ptr<sstable_write_progress> wp) override {
        _in_progress.erase(std::move(wp));
    }

    void register_compacting_sstable(lw_shared_ptr<compaction_read_progress> rp) override {
        _in_compaction.insert(std::move(rp));
    }

    void finish_compacting_sstable(lw_shared_ptr<compaction_read_progress> rp) override {
        _in_compaction.erase(std::move(rp));
    }
};
...
seastar::lw_shared_ptr<compaction_backlog_tracker> get_unimplemented_backlog_tracker() {
    static thread_local seastar::lw_shared_ptr<compaction_backlog_tracker> unimplemented = make_backlog_tracker(std::make_unique<unimplemented_backlog_tracker>());
...
seastar::lw_shared_ptr<compaction_backlog_tracker> get_null_backlog_tracker() {
    static thread_local seastar::lw_shared_ptr<compaction_backlog_tracker> nulltracker = make_backlog_tracker(std::make_unique<null_backlog_tracker>());
    return nulltracker;
}
...
size_tiered_compaction_strategy::size_tiered_compaction_strategy(const std::map<sstring, sstring>& options)
    ...
    , _options(options)
    , _backlog_tracker(make_backlog_tracker(std::make_unique<size_tiered_backlog_tracker>()))
{}
...
seastar::lw_shared_ptr<compaction_backlog_tracker> compaction_strategy::get_backlog_tracker() {

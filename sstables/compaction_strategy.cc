...
class size_tiered_backlog_tracker final: public compaction_backlog_tracker::impl {
    ...
    std::unordered_map<sstables::shared_sstable, shared_ptr<backlog_write_progress_manager>> _in_progress = {};
    std::unordered_map<sstables::shared_sstable, shared_ptr<backlog_read_progress_manager>> _in_compaction = {};
...
public:
    virtual compaction_backlog_tracker::backlog get_backlog() override {
        ...
        // We will normalize the backlog relative to memory size. We will establish that a backlog
        // 10 times high than memory size is equal to "1", IOW a high backlog.
        auto ret = ((total_bytes * log4(total_bytes)) - sstables_contribution) / (10 * memory::stats().total_memory());
        return compaction_backlog_tracker::backlog{ret , total_bytes};
    }
    ...
    virtual void add_sstable(sstables::shared_sstable sst)  override {
        ...
        auto it = _in_progress.find(sst);
        if (it != _in_progress.end()) {
            _in_progress.erase(it);
        }
    }
    ...
    virtual void remove_sstable(sstables::shared_sstable sst)  override {
        ...
        auto it = _in_compaction.find(sst);
        if (it != _in_compaction.end()) {
            _in_compaction.erase(it);
        }
    }

    virtual void register_partially_written_sstable(sstables::shared_sstable sst, shared_ptr<backlog_write_progress_manager> wp) override {
        _in_progress.emplace(sst, wp);
    }

    virtual void register_compacting_sstable(sstables::shared_sstable sst, shared_ptr<backlog_read_progress_manager> rp) override {
        _in_compaction.emplace(sst, rp);
    }
};

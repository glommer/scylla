...
class time_window_backlog_tracker final : public compaction_backlog_tracker::impl {
    ...
    mutable std::unordered_map<api::timestamp_type, size_tiered_backlog_tracker> _backlog_per_window;
...
public:
    ...
    virtual double backlog(const compaction_backlog_tracker::ongoing_writes& ow, const compaction_backlog_tracker::ongoing_compactions& oc) const override {
        ...
        for (auto& wp : ow) {
            ...
            auto it = writes_per_window.find(bound);
            if (it == writes_per_window.end()) {
                auto ret = writes_per_window.emplace(bound, compaction_backlog_tracker::ongoing_writes());
                it = ret.first;
            }
            it->second.insert(wp);
        }
        ...
        for (auto& cp : oc) {
            ...
            if (it == compactions_per_window.end()) {
                auto ret = compactions_per_window.emplace(bound, compaction_backlog_tracker::ongoing_compactions());
                it = ret.first;
            }
            it->second.insert(cp);
        }
        ...
        auto it = _backlog_per_window.begin();
        for (auto& windows : _backlog_per_window) {
            auto bound = windows.first;
            auto ow_this_window = std::reference_wrapper<compaction_backlog_tracker::ongoing_writes>(no_ow);
            auto itw = writes_per_window.find(bound);
            if (itw != writes_per_window.end()) {
                ow_this_window = std::reference_wrapper<compaction_backlog_tracker::ongoing_writes>(itw->second);
                writes_per_window.erase(itw);
            }
            auto oc_this_window = std::reference_wrapper<compaction_backlog_tracker::ongoing_compactions>(no_oc);
            auto itc = compactions_per_window.find(bound);
            if (itc != compactions_per_window.end()) {
                oc_this_window = std::reference_wrapper<compaction_backlog_tracker::ongoing_compactions>(itc->second);
            }
            b += windows.second.backlog(ow_this_window, oc_this_window);
        }

        // ongoing writes for the current window.
        // Should loop only once but let's still write it as a for loop to cover our bases.
        //
        // No need to do the same for ongoing compactions. If they are compacting they have to cover
        // an existing SSTable.
        for (auto& current : writes_per_window) {
            auto ret = _backlog_per_window.emplace(current.first, size_tiered_backlog_tracker());
            it = ret.first;
            b += it->second.backlog(current.second, no_oc);
        }
        return b;
    }
    ...
    virtual void add_sstable(sstables::shared_sstable sst) override {
        ...
        auto it = _backlog_per_window.find(bound);
        if (it == _backlog_per_window.end()) {
            auto ret = _backlog_per_window.emplace(bound, size_tiered_backlog_tracker());
            it = ret.first;
        }
        it->second.add_sstable(sst);
    }
    ...
    virtual void remove_sstable(sstables::shared_sstable sst) override {
        auto bound = lower_bound_of(sst->get_stats_metadata().max_timestamp);
        auto it = _backlog_per_window.find(bound);
        // This can happen for instance if we are not tracking this window anymore (too old an
        // no backlog) and then we drop the table.
        if (it != _backlog_per_window.end()) {

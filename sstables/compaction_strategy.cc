...
public:
    ...
    virtual double backlog(const compaction_backlog_tracker::ongoing_writes& ow, const compaction_backlog_tracker::ongoing_compactions& oc) const override {
        ...
        //
        // The fan_out is usually 10 (sstables_per_run). But if the level above us is not
        ...
        for (size_t level = 0; level < _size_per_level.size() - 1; ++level) {
            ...
            for (size_t next = level + 1; next < _size_per_level.size() - 1; ++next) {
                ...
                b += std::min(double(leveled_manifest::sstables_per_run), double(lsize_next) / _max_sstable_size)  * lsize;
            }
        }
        return b;
    }

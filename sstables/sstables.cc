...
read_monitor& default_read_monitor() {
    // Same as the default_write_monitor: not keeping state, so let's not use thread_local
    static noop_read_monitor monitor;
    return monitor;
}

read_monitor_generator& default_read_monitor_generator() {
    // Same as the previous two monitors not keeping state, so let's not use thread_local
    static no_read_monitoring generator;
    return generator;
}
...
void sstable_writer::consume_end_of_stream()
{
    ...
    _monitor.on_data_write_completed(_writer->offset_tracker());
    finish_file_writer();

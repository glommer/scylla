...
future<> sstable::write_components(memtable& mt, const io_priority_class& pc) {
    _collector.set_replay_position(mt.replay_position());
    return write_components(mt.make_reader(mt.schema()),
            mt.partition_count(), mt.schema(), std::numeric_limits<uint64_t>::max(), pc);
}
...
future<> sstable::write_components(::mutation_reader mr,
        ...
        const io_priority_class& pc) {
    return seastar::async([this, mr = std::move(mr), estimated_partitions, schema = std::move(schema), max_sstable_size, &pc] () mutable {
...
future<> sstable::mutate_sstable_level(uint32_t new_level) {
    ...
    return seastar::async([this] {
        ...
        // priority enough so we will use the compaction priority directly.
        auto& pc = service::get_local_priority_manager().compaction_priority();
        write_statistics(pc);
    });
}

...
compaction_manager::~compaction_manager() {
    ...
    assert(_state == state::none || _state == state::destroyed);
}
...
future<> compaction_manager::drain() {
    _state = state::disabled;
    cmlog.info("Draining {} ongoing compactions", _compactions.size());
...
void compaction_manager::do_stop() {
    if (_state == state::none || _state == state::destroyed) {
        return;
    }
    ...
    _metrics.clear();
    _stop_future.emplace(drain().then([this] () mutable {
        reevaluate_postponed_compactions();

...
future<> compaction_manager::run_custom_job(column_family* cf, sstring name, std::function<future<>()> job) {
    if (_state != state::enabled) {
        return make_ready_future<>();
    }
    ...
    task->compaction_done = with_semaphore(_custom_job_sem, 1, [this, task, cf, job = std::move(job)] {

...
struct compaction_read_monitor_generator final : public read_monitor_generator {
    ...
    public:
        virtual void on_read_started(const sstables::reader_position_tracker& tracker) override {
            _tracker = &tracker;
            _backlog_tracker->register_compacting_sstable(_sst, shared_from_this());
        }
    ...
    virtual shared_ptr<sstables::read_monitor> operator()(sstables::shared_sstable sst) override {
...
public:
    compaction_write_monitor(sstables::shared_sstable sst, lw_shared_ptr<compaction_backlog_tracker> backlog_tracker)
        : _sst(sst)
    ...
    virtual void on_write_started(const sstables::writer_offset_tracker& tracker) override {
        _tracker = &tracker;
        _backlog_tracker->register_partially_written_sstable(_sst, shared_from_this());
    }
...
public:
    ...
    virtual sstable_writer* select_sstable_writer(const dht::decorated_key& dk) override {
        if (!_writer) {
            ...
            _active_write_monitors.emplace_back(make_shared<compaction_write_monitor>(_sst, _backlog_tracker));
...
public:
    ...
    sstable_writer* select_sstable_writer(const dht::decorated_key& dk) override {
        ...
        if (!writer) {
            ...
            _active_write_monitors.emplace_back(make_shared<compaction_write_monitor>(sst, _backlog_tracker));

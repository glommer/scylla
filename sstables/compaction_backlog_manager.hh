...
// the sstable-side structures go out of scope. And in that case we can do something about it.
//
// We will deal with shared pointers for our progress manager objects because they are expected to
// outlive the current compaction and/or write. The reason for that, is that if we unregister a
// progrees manager and only later on add/remove the corresponding SSTable we can see large valley or
// peaks in the backlog.
//
...
struct backlog_write_progress_manager : public enable_shared_from_this<backlog_write_progress_manager> {
    virtual uint64_t written() const = 0;
};

struct backlog_read_progress_manager  : public enable_shared_from_this<backlog_read_progress_manager> {
...
public:
    ...
    struct impl {
        ...
        virtual void register_partially_written_sstable(sstables::shared_sstable sst, shared_ptr<backlog_write_progress_manager> wp) = 0;
        virtual void register_compacting_sstable(sstables::shared_sstable sst, shared_ptr<backlog_read_progress_manager> rp) = 0;
    ...
    void register_partially_written_sstable(sstables::shared_sstable sst, shared_ptr<backlog_write_progress_manager> wp);
    void register_compacting_sstable(sstables::shared_sstable sst, shared_ptr<backlog_read_progress_manager> rp);
...
private:
    ...
    std::unordered_map<sstables::shared_sstable, shared_ptr<backlog_write_progress_manager>> _ongoing_writes;

...
public:
    // This is not a virtual function for performance reasons.
    std::function<proceed (temporary_buffer<char>&)> _process;
    continuous_data_consumer(input_stream<char>&& input, uint64_t maxlen, std::function<proceed (temporary_buffer<char>&)>&& process)
            : _input(std::move(input)), _remain(maxlen) , _process(std::move(process)) {}
    ...
    proceed process(temporary_buffer<char>& data) {

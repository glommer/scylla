...
future<> storage_service::take_column_family_snapshot(sstring ks_name, sstring cf_name, sstring tag) {
    ...
    return smp::submit_to(0, [] {
        ...
        return _db.invoke_on_all([ks_name, tag] (database &db) {
            check_snapshot_not_exist(db, tag, ks_name);
            return make_ready_future<>();
        }).then([this, ks_name, cf_name, tag] {
...
future<> storage_service::clear_snapshot(sstring tag, std::vector<sstring> keyspace_names) {
    ...
    }).finally([deleted_keyspaces, this, tag] {
        return _db.invoke_on_all([tag, deleted_keyspaces] (database& db) {
            return parallel_for_each(*deleted_keyspaces, [&db, tag] (auto ks_name) {
                auto& ks = db.find_keyspace(ks_name);
                if (tag.empty()) {
                    ks.remove_all_snapshots();
                } else {
                    ks.remove_snapshot(tag);
                }
                return make_ready_future<>();
            });
        });

...
void set_storage_service(http_context& ctx, routes& r) {
    ...
    ss::take_snapshot.set(r, [](std::unique_ptr<request> req) {
        ...
        std::vector<sstring> keynames;
        for (auto&& s : split(req->get_query_param("kn"), ",")) {
            keynames.emplace_back(std::move(s));
        }

        if (keynames.size() == 0) {
            throw httpd::bad_param_exception("expected at least one keyspace");
        }

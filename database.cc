        ...
        with_lock(_sstables_lock.for_read(), [this, old, permit = std::move(permit)] () mutable {
            ...
            return do_with(std::move(monitor), [this, newtab, old, permit = std::move(permit)] (auto& monitor) mutable {
                ...
                return write_memtable_to_sstable(*old, newtab, monitor, incremental_backups_enabled(), priority, false).then([this, newtab, old] {
                  ...
                  // Switch back to default scheduling group for the compaction trigger and for the cache update. The
                  // Cache update does not affect the input of the memtable cpu controller, so it can be subject to
                  // priority inversion.
                  return with_scheduling_group(default_scheduling_group(), [this, old = std::move(old), newtab = std::move(newtab)] () mutable {
                    auto adder = [this, newtab] {
                        add_sstable(newtab, {engine().cpu_id()});
                        try_trigger_compaction();
                        dblog.debug("Flushing to {} done", newtab->get_filename());
                    };
                    return with_scheduling_group(_config.memtable_to_cache_scheduling_group, [this, adder, old] {
                      if (_config.enable_cache) {
...
column_family::compact_sstables(sstables::compaction_descriptor descriptor, bool cleanup) {
    ...
    return with_lock(_sstables_lock.for_read(), [this, descriptor = std::move(descriptor), cleanup] () mutable {
        ...
        return sstables::compact_sstables(std::move(descriptor), *this, create_sstable,
                cleanup, _config.compaction_scheduling_group).then([this, sstables_to_compact = std::move(sstables_to_compact)] (auto info) {
...
database::database(const db::config& cfg, database_config dbcfg)
    ...
    , _dbcfg(dbcfg)
    , _memtable_cpu_controller(make_flush_controller(*_cfg, dbcfg.memtable_scheduling_group, [this, limit = 2.0f * _dirty_memory_manager.throttle_threshold()] {
    ...
    , _flush_io_controller(make_flush_controller(*_cfg, service::get_local_memtable_flush_priority(), [this, limit = 2.0f * _dirty_memory_manager.throttle_threshold()] {
        return _dirty_memory_manager.virtual_dirty_memory() / limit;
    }))

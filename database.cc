...
column_family::sstables_as_mutation_source() {
    return [this] (schema_ptr s, const query::partition_range& r, const io_priority_class& pc) {
...
key_source column_family::sstables_as_key_source() const {
    return [this] (const query::partition_range& range, const io_priority_class& pc) {
...
column_family::find_partition(schema_ptr s, const dht::decorated_key& key) const {
    return do_with(query::partition_range::make_singular(key), [s = std::move(s), this] (auto& range) {
        return do_with(this->make_reader(s, default_priority_class(), range), [] (mutation_reader& reader) {
...
mutation_reader
column_family::make_reader(schema_ptr s, const io_priority_class& pc, const query::partition_range& range) const {
    ...
    if (_config.enable_cache) {
        readers.emplace_back(_cache.make_reader(s, pc, range));
    } else {
...
column_family::for_all_partitions(schema_ptr s, Func&& func) const {
    ...
    public:
        ...
        iteration_state(schema_ptr s, const column_family& cf, Func&& func)
            : reader(cf.make_reader(std::move(s), default_priority_class()))
            , func(std::move(func))
        { }
    };
...
column_family::as_mutation_source() const {
    return [this] (schema_ptr s, const query::partition_range& range, const io_priority_class& pc) {
        return this->make_reader(std::move(s), pc, range);
    };
}

...
struct column_family::merge_comparator {
    ...
    bool operator()(ptr x, ptr y) const {
        return !x->front().first.less_compare(*_schema, y->front().first);
    }
};
...
bool
column_family::testonly_for_all_partitions(std::function<bool (const dht::decorated_key&, const mutation_partition&)> func) const {

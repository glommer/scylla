...
column_family::reshuffle_sstables(unsigned long start) {
    struct work {
        unsigned long current_gen;
        sstable_list sstables;
        std::unordered_map<unsigned long, sstables::entry_descriptor> descriptors;
        std::vector<sstables::entry_descriptor> reshuffled;
        work(unsigned long start) : current_gen(start ? start : 1) {}
    };
...
column_family::compact_sstables(std::vector<sstables::shared_sstable> sstables) {
    ...
    return with_lock(_sstables_lock.for_read(), [this, sstables = std::move(sstables)] {

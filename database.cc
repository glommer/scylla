...
public:
    ...
    virtual void on_write_started(const sstables::writer_offset_tracker& t) override {
        _tracker = &t;
        _compaction_strategy.get_backlog_tracker()->register_partially_written_sstable(_sst, shared_from_this());
    }
...
column_family::seal_active_streaming_memtable_immediate(flush_permit&& permit) {
    ...
    return with_gate(_streaming_flush_gate, [this, old, permit = std::move(permit)] () mutable {
        ...
        with_lock(_sstables_lock.for_read(), [this, old, permit = std::move(permit)] () mutable {
            ...
            auto monitor = seastar::make_shared<database_sstable_write_monitor>(permit.release_sstable_write_permit(), newtab, _compaction_manager, _compaction_strategy);
...
column_family::try_flush_memtable_to_sstable(lw_shared_ptr<memtable> old, sstable_write_permit&& permit) {
    ...
    auto monitor = seastar::make_shared<database_sstable_write_monitor>(std::move(permit), newtab, _compaction_manager, _compaction_strategy);
...
void column_family::set_compaction_strategy(sstables::compaction_strategy_type strategy) {
    ...
    _compaction_strategy.get_backlog_tracker()->transfer_ongoing_writes(new_cs.get_backlog_tracker());
...
void backlog_controller::adjust() {
    ...
    float result = 0;
    if (backlog < _thresholds.first_threshold) {
        result = backlog * _slopes.q1 / _thresholds.first_threshold;
    } else if ((backlog >= _thresholds.first_threshold) && (backlog < _thresholds.second_threshold)) {
        result = _slopes.q1 + (backlog - _thresholds.first_threshold) * (_slopes.q2 - _slopes.q1)/(_thresholds.second_threshold - _thresholds.first_threshold);
    } else {
        result = _slopes.q2 + (backlog - _thresholds.second_threshold) * (_slopes.qmax - _slopes.q2) / (_thresholds.maximum - _thresholds.second_threshold);
    }
...
void backlog_io_controller::update_controller(float shares) {
    with_gate(_update_gate, [this, shares] {
        return engine().update_shares_for_class(_io_priority, uint32_t(shares));
    });
}
        ...
        sstables::read_monitor_generator& monitor_generator)
{
    auto reader_factory_fn = [s, &slice, &pc, resource_tracker, fwd, fwd_mr, &monitor_generator] (sstables::shared_sstable& sst, const dht::partition_range& pr) {
        mutation_reader reader = mutation_reader_from_flat_mutation_reader(sst->read_range_rows_flat(s, pr, slice, pc, resource_tracker, fwd, fwd_mr, monitor_generator(sst)));
        if (sst->is_shared()) {
...
{
    auto reader_factory_fn = [s, &slice, &pc, resource_tracker, fwd, fwd_mr, &monitor_generator] (sstables::shared_sstable& sst, const dht::partition_range& pr) {
        return mutation_reader_from_flat_mutation_reader(sst->read_range_rows_flat(s, pr, slice, pc, resource_tracker, fwd, fwd_mr, monitor_generator(sst)));
    };
    return mutation_reader_from_flat_mutation_reader(
            make_flat_mutation_reader<combined_mutation_reader>(s, std::make_unique<incremental_reader_selector>(s,
                    std::move(sstables),

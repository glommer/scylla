...
void backlog_cpu_controller::adjust() {
    ...
    if (backlog < _first_threshold) {
        _current_quota = backlog * q1 / _first_threshold;
    } else if ((backlog >= _first_threshold) && (backlog < _second_threshold)) {
        _current_quota = q1 + (backlog - _first_threshold) * (q2 - q1)/(_second_threshold - _first_threshold);
    } else {
        _current_quota = q2 + (backlog - _second_threshold) * (qmax - q2) / (_maximum - _second_threshold);
    }

...
column_family::sstables_as_snapshot_source() {
    return snapshot_source([this] () {
        ...
        return mutation_source([this, sst_set] (schema_ptr s,
            ...
            return make_sstable_reader(std::move(s), sst_set, r, slice, pc, std::move(trace_state), fwd, fwd_mr, _config.read_concurrency_config);
        }, [this, sst_set] {
...
column_family::make_sstable_reader(schema_ptr s,
                                   ...
                                   streamed_mutation::forwarding fwd,
                                   mutation_reader::forwarding fwd_mr,
                                   restricted_mutation_reader_config config) const {
...
column_family::make_streaming_reader(schema_ptr s,
    ...
    auto source = mutation_source([this] (schema_ptr s, const dht::partition_range& range, const query::partition_slice& slice,
        ...
        readers.emplace_back(make_sstable_reader(s, _sstables, range, slice, pc, std::move(trace_state), fwd, fwd_mr, _config.streaming_read_concurrency_config));
        return make_combined_reader(std::move(readers), fwd_mr);
    });
...
column_family::query(schema_ptr s, const query::read_command& cmd, query::result_request request,
                     ...
                     uint64_t max_size, column_family::timeout_clock::time_point timeout) {
    ...
    return f.then([this, lc, s = std::move(s), &cmd, request, &partition_ranges, trace_state = std::move(trace_state), timeout] (query::result_memory_accounter accounter) mutable {
        ...
        return do_until(std::bind(&query_state::done, &qs), [this, &qs, trace_state = std::move(trace_state), timeout] {
            ...
            auto db_timeout = timeout > now ? timeout - now : 0ms;
            return data_query(qs.schema, as_mutation_source(db_timeout), range, qs.cmd.slice, qs.remaining_rows(),
...
database::query(schema_ptr s, const query::read_command& cmd, query::result_request request, const dht::partition_range_vector& ranges, tracing::trace_state_ptr trace_state,
                uint64_t max_result_size, timeout_clock::time_point timeout) {
...
database::query_mutations(schema_ptr s, const query::read_command& cmd, const dht::partition_range& range,
                          query::result_memory_accounter&& accounter, tracing::trace_state_ptr trace_state, column_family::timeout_clock::time_point timeout) {
    column_family& cf = find_column_family(cmd.cf_id);
    auto now = timeout_clock::now();
    auto db_timeout = timeout > now ? timeout - now : 0ms;
    return mutation_query(std::move(s), cf.as_mutation_source(db_timeout), range, cmd.slice, cmd.row_limit, cmd.partition_limit,

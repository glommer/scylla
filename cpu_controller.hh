...
private:
    ...
    void adjust();
protected:
    backlog_cpu_controller(disabled d) : _scheduling_group(d.backup) {}
    backlog_cpu_controller(backlog_cpu_controller&&) = default;

    backlog_cpu_controller(std::chrono::milliseconds interval, seastar::scheduling_group sg, float ft, float st, float max, std::function<float()> backlog)
        : _interval(interval)

...
// active segment fills up, it is closed. Closed segmenets are kept in a heap
...
class region::impl : public allocation_strategy {
    ...
    struct mover {
        impl& self;
        object_descriptor* desc;

        template <typename T>
        void operator()(T* obj) {
            auto dst = self.alloc_small(desc->migrator(), desc->size(), desc->alignment());
...
private:
    void* alloc_large(size_t size, uint8_t alignment) {
    ...
    void* alloc_small(allocation_strategy::migrate_fn migrator, segment::size_type size, uint8_t alignment) {
        ...
        size_t obj_offset = align_up(_active_offset + sizeof(object_descriptor), (size_t) alignment);
    ...
    void for_each_live(segment* seg, Func&& func) {
        ...
        while (offset < segment::size) {
            ...
            if (desc->is_live()) {
                func(desc, reinterpret_cast<void*>(seg->data + offset + sizeof(object_descriptor)));
            }
            offset += sizeof(object_descriptor) + desc->size();
        }
    }
...
public:
    ...
    //
    // Returns true if this pool can be compacted so that compact() will make forward progress,
    // Forward progress here means that this expression must eventually stop:
    //
    ...
    virtual void* alloc(allocation_strategy::migrate_fn migrator, size_t size, size_t alignment) override {
        ...
        if (size > max_managed_object_size) {
            return alloc_large(size, (uint8_t) alignment);
        } else {
            return alloc_small(migrator, (segment::size_type) size, (uint8_t) alignment);
        }
    }
...
bool region::compactible() const {
    return _impl->compactible();
}
...
uint64_t tracker::impl::reclaim(uint64_t bytes) {
    ...
    // Algorithm outline.
    //
    // Regions are kept in a max-heap ordered so that regions with the
    // sparsest segment are picked first. Non-compactible regions will be
    ...
    if (logger.is_enabled(logging::log_level::debug)) {
        ...
        for (region::impl* r : _regions) {
            logger.debug(" - {}: min={}, avg={}", r->id(), r->min_occupancy(), r->occupancy());
        }
    }

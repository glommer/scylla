...
class managed {
    ...
    template<typename T_, typename AllocationStrategy_>
...
// allocated object.
//
template<typename T, typename AllocationStrategy, typename... Args>
managed_ref<T, AllocationStrategy>
make_managed(Args&&... args) {
    managed_ref<T, AllocationStrategy> ref;
    AllocationStrategy().template construct<managed<T>>(&ref._ptr, T(std::forward<Args>(args)...));
    return ref;
}

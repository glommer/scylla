...
SEASTAR_TEST_CASE(test_region_groups_basic_throttling) {
    return seastar::async([] {
        ...
        with_allocator(big_region->allocator(), [&] {
            std::vector<managed_bytes>().swap(big_alloc);
        });
        big_region.reset();
        ...
        with_allocator(simple_region->allocator(), [&simple_objs] {
            std::vector<managed_ref<uint64_t>>().swap(simple_objs);
        });
        simple.shutdown().get();
    });
}
...
SEASTAR_TEST_CASE(test_region_groups_linear_hierarchy_throttling_child_alloc) {
    return seastar::async([] {
        ...
        with_allocator(parent_region->allocator(), [&small_alloc] {
            std::vector<managed_ref<uint64_t>>().swap(small_alloc);
        });
        parent_region.reset();
...
SEASTAR_TEST_CASE(test_region_groups_basic_throttling_active_reclaim_leaf_offender) {
    return seastar::async([] {
        ...
        root.rg().shutdown().get();
        large_leaf.rg().shutdown().get();
        small_leaf.rg().shutdown().get();
    });
}

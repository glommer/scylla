...
SEASTAR_TEST_CASE(datafile_generation_17) {
    return test_setup::do_with_test_directory([] {
        ...
        auto sst = make_lw_shared<sstable>("tests/urchin/sstables/tests-temporary", 17, la, big);
        return sst->write_components(*mtp).then([s] {
            return reusable_sst("tests/urchin/sstables/tests-temporary", 17).then([s] (auto sstp) {
...
SEASTAR_TEST_CASE(datafile_generation_18) {
    return test_setup::do_with_test_directory([] {
        ...
        auto sst = make_lw_shared<sstable>("tests/urchin/sstables/tests-temporary", 18, la, big);
        return sst->write_components(*mtp).then([s] {
            return reusable_sst("tests/urchin/sstables/tests-temporary", 18).then([s] (auto sstp) {
                return do_with(sstables::key("key1"), [sstp, s] (auto& key) {
                    return sstp->read_row(s, key).then([sstp, s] (auto mutation) {
                        auto& mp = mutation->partition();
...
SEASTAR_TEST_CASE(datafile_generation_19) {
    return test_setup::do_with_test_directory([] {
        ...
        auto sst = make_lw_shared<sstable>("tests/urchin/sstables/tests-temporary", 19, la, big);
        return sst->write_components(*mtp).then([s] {
            return reusable_sst("tests/urchin/sstables/tests-temporary", 19).then([s] (auto sstp) {
                return do_with(sstables::key("key1"), [sstp, s] (auto& key) {
                    return sstp->read_row(s, key).then([sstp, s] (auto mutation) {
                        auto& mp = mutation->partition();
                        auto exploded = exploded_clustering_prefix({"cl1", "cl2"});

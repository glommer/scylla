...
SEASTAR_TEST_CASE(test_flush_in_the_middle_of_a_scan) {
    ...
    return with_column_family(s, cfg, [s](column_family& cf) {
        return seastar::async([s, &cf] {
            ...
            auto assert_that_scanner1 = assert_that(cf.make_reader(s, default_priority_class(), query::full_partition_range));

            // Flush will happen before it is invoked
            auto assert_that_scanner2 = assert_that(cf.make_reader(s, default_priority_class(), query::full_partition_range));

            // Flush will happen after all data was read, but before EOS was consumed
            auto assert_that_scanner3 = assert_that(cf.make_reader(s, default_priority_class(), query::full_partition_range));

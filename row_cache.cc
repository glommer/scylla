...
mutation_reader
row_cache::make_scanning_reader(schema_ptr s, const io_priority_class& pc, const query::partition_range& range) {
...
mutation_reader
row_cache::make_reader(schema_ptr s, const io_priority_class& pc, const query::partition_range& range) {
    if (range.is_singular()) {
        const query::ring_position& pos = range.start()->value();

        if (!pos.has_key()) {
            return make_scanning_reader(std::move(s), pc, range);
        }

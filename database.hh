...
protected:
    ...
    virtual future<> flush_when_needed();

    // Permit, Parent Permit, and Global Pressure:
    // -------------------------------------------
    //
    ...
    //
    // Also, if a region is flushing and we happen to be under global pressure, that may be enough
    // to bring relief so we don't want any other region to initiate a flush at this time. To
    // guarantee that, every time we start a flush, we acquire units for the parent, and store them
    // at parent_permit. Note that those units are acquired by consume(), which can bring the
    // semaphore's total usage to at most 3 (nr_regions) if all regions are flushing at the same
    // time and in the absence of global pressure. That means that the parent (overall container)
    // will only be able to acquire its own semaphore when no other child is flushing. In which case
    // it will pick the best and initiate a flush.
    //
    // The consumption of an unit of the parent's semaphore happens unconditionally for every flush,
    // to account for explicit flushes. But if the parent container initiated a flush it will have
    // acquired a permit itself. That permit must be kept until the flush actually starts. The
    // parent will store that permit in the _global_pressure pressure attribute for the child. That
    // also has the role of signalling to the child that pressure exists and it should flush, even
    // if its own region is not under pressure.
    struct flush_permit {
        semaphore_units<> permit;
        semaphore_units<> parent_permit;

        flush_permit(semaphore_units<>&& permit, semaphore_units<>&& parent_permit)
                : permit(std::move(permit)), parent_permit(std::move(parent_permit)) {}
    };
    ...
    bool has_pressure() const {
        return over_soft_limit() || bool(_global_pressure);
    }

    std::experimental::optional<semaphore_units<>> _global_pressure;
public:
    future<> shutdown();

    void notify_global_pressure(semaphore_units<> perm) {
        _global_pressure = std::move(perm);
    ...
    //
    // The container has a slightly higher soft limit (82 %). If we were to set it to the same value
    // than the leaf groups, we would be waking up the container flusher unnecessarily in the case
    // where only one of the groups is active - its own pressure should suffice to trigger a flush.
    //
    ...
    dirty_memory_manager(dirty_memory_manager *parent) : logalloc::region_group_reclaimer()
        , _db(nullptr)
        , _region_group(&parent->_region_group, *this)
        , _flush_serializer(1)
        , _waiting_flush(make_ready_future<>()) {}
...
// Like the dirty memory manager, but won't hold memtables. Exists solely to
// enforce global limits in the children.
class dirty_memory_container : public dirty_memory_manager {
public:
    dirty_memory_container() : dirty_memory_manager() {}
    dirty_memory_container(database& db, size_t threshold) : dirty_memory_manager(db, threshold) {
        _waiting_flush = flush_when_needed();
    }
    virtual future<> flush_when_needed() override;
};

extern thread_local dirty_memory_container default_dirty_memory_container;
...
private:
    ...
    dirty_memory_container _overall_dirty_memory_container;

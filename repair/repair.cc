...
static future<> repair_cf_range(seastar::sharded<database>& db,
    ...
    // We need a second semaphore because we are not waiting for the inner-block in do_for_each. Since we are using
    // a lot of reference variables here, we need to make sure that do_for_each only truly returns when everybody
    // is done.
    return do_with(semaphore(0), true, std::move(keyspace), std::move(cf), std::move(ranges),
        [&db, &neighbors] (auto& completion_sem, auto& success, const auto& keyspace, const auto& cf, const auto& ranges) {
        return do_for_each(ranges, [&completion_sem, &success, &db, &neighbors, &keyspace, &cf]
                           (const auto& range) {

            check_in_shutdown();
            return parallelism_semaphore.wait(1).then([&completion_sem, &success, &db, &neighbors, &keyspace, &cf, &range] {
                ...
                }).finally([&completion_sem] {
                    parallelism_semaphore.signal(1);
                    completion_sem.signal(1); // notify do_for_each that we're done
                });
            });
        }).finally([&success, &completion_sem, pending = ranges.size()] {
            return completion_sem.wait(pending).then([&success, &completion_sem] {
